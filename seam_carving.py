...
def seams_removal(im, num_remove, file_path, mask=None, vis=False, rot=False):
    if not rot:
        file_name=str(file_path.rsplit('.', 1)[0])+"seams.pkl"
    else:
        file_name=str(file_path.rsplit('.', 1)[0])+"seams_rotated.pkl"
    if os.path.isfile(file_name):
        seams_record,boolmask_record = pickle.load( open( file_name, "rb" ) )
    else:
        precompute_seams(im,file_path,mask,rot)
        seams_record,boolmask_record = pickle.load( open( file_name, "rb" ) )
    
    seams_record.reverse()
    boolmask_record.reverse()

    for _ in range(num_remove):
        seam_idx = seams_record.pop()
        boolmask = boolmask_record.pop()
...
def precompute_seams(im,file_path,mask=None,rot=False):
    seams_record = []
    boolmask_record=[]
    temp_im = im.copy()
    temp_mask = mask.copy() if mask is not None else None
    num_seams=im.shape[1]
    if not rot:
        print("Precomputing vertical seams")
        file_name=str(file_path.rsplit('.', 1)[0])+"seams.pkl"
    else:
        print("Precomputing horizontal seams")
        file_name=str(file_path.rsplit('.', 1)[0])+"seams_rotated.pkl"
    for _ in range(num_seams):
        seam_idx, boolmask = get_minimum_seam(temp_im, temp_mask)
        seams_record.append(seam_idx)
        boolmask_record.append(boolmask)
        temp_im = remove_seam(temp_im, boolmask)
        if temp_mask is not None:
            temp_mask = remove_seam_grayscale(temp_mask, boolmask)
        
    pickle.dump( [seams_record, boolmask_record], open( file_name, "wb" ) )
    return seams_record
    

def seams_insertion(im, num_add, file_path, mask=None, vis=False, rot=False):
    if not rot:
        file_name=str(file_path.rsplit('.', 1)[0])+"seams.pkl"
    else:
        file_name=str(file_path.rsplit('.', 1)[0])+"seams_rotated.pkl"
...
def seam_carve(im, dy, dx, file_path, mask=None, vis=False):
    im = im.astype(np.float64)
    h, w = im.shape[:2]
    print("h:",h,"w:",w,"dx:",dx,"dy:",dy)
    ...
    if dx < 0:
        output, mask = seams_removal(output, -dx, file_path, mask, vis)

    elif dx > 0:
        output, mask = seams_insertion(output, dx, file_path, mask, vis)

    if dy < 0:
        output = rotate_image(output, True)
        if mask is not None:
            mask = rotate_image(mask, True)
        output, mask = seams_removal(output, -dy, file_path, mask, vis, rot=True)
        ...
        if mask is not None:
            mask = rotate_image(mask, True)
        output, mask = seams_insertion(output, dy, file_path, mask, vis, rot=True)
        output = rotate_image(output, False)
