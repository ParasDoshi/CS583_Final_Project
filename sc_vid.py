# Originally implemented by Github's andrewdcampbell
...
def seams_insertion2(im, seams, mask=None, vis=False, rot=False):
    seams_record = seams
...
def seam_carve(im, dy, dx, mask=None, vis=False):
    ...
    if dx < 0:
        output, mask, vs = seams_removal(output, -dx, mask, vis)

    elif dx > 0:
        output, mask, vs = seams_insertion(output, dx, mask, vis)

    if dy < 0:
        output = rotate_image(output, True)
        if mask is not None:
            mask = rotate_image(mask, True)
        output, mask, hs = seams_removal(output, -dy, mask, vis, rot=True)
        ...
        if mask is not None:
            mask = rotate_image(mask, True)
        output, mask, hs = seams_insertion(output, dy, mask, vis, rot=True)
        output = rotate_image(output, False)
